...
if __name__ == "__main__":
    ...
    # Switch off channel output before confuguring it
    card.outputOff()
    ...
    filename = "ExampleWaveform.sig"
    waveform = card.readWaveformFromFile(filename)

    try:
        # Load the waveform into the card memory.
        # Repetition rate (frequency) is expressed as a float value in kHz
        # and passed as an optional parameter to loadArbitraryWaveform()
        card.loadArbitraryWaveform(waveform, repetitionRate=1.0)
    ...
    # Close the session with the card. This will not stop the function generator.
    card.close()